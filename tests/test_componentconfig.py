...
def test_GaussianComponentConfig(centroid):
    ...
    for idx, component_data in enumerate(components):
        ...
        fracs = [
            param
            for param in get_params_uniq(component, linear=False)
...
def test_SersicConfig(centroid, channels):
    ...
    fluxes = {channel: 1.0 + idx for idx, channel in enumerate(channels.values())}
    ...
    fluxes_label = {
        config.format_label(config.get_integral_label_default(), name_channel=channel.name): fluxes[channel]
        for channel in fluxes.keys()
    }
    for param in params:
