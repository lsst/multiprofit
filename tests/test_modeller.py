                            ...
                            **(
                                {
                                    ...
                                    "fluxfrac": FluxFractionParameterConfig(
                                        value_initial=frac_chan, fixed=False
                                    ),
                                }
                                if (idx == 0)
                                else {}
...
def model(channels, data, psf_models) -> g2f.ModelD:
    ...
    is_linear_interp = (
        g2f.SersicMixComponentIndexParameterD(
            interpolator=SersicComponentConfig().get_interpolator(4)
        ).interptype
        == g2f.InterpType.linear
    )
...
def psf_observations(psf_models) -> list[g2f.ObservationD]:
    ...
    for psf_model in psf_models:
        ...
        for idx in range(len(gaussians_source)):
            ...
            gaussian_idx.centroid.x = image.n_cols / 2.0
            gaussian_idx.centroid.y = image.n_rows / 2.0
...
def test_model_evaluation(channels, model, model_func_scope):
    ...
    for m in (model, model_func_scope):
        for psf_model in m.psfmodels:
            params = psf_model.parameters()
            for param in params:
                param.fixed = True

    model.setup_evaluators(print=printout, force=True)
    model.evaluate()
    ...
    params_free = tuple(get_params_uniq(model_func_scope, fixed=False))
    ...
    model_func_scope.setup_evaluators(
        evaluatormode=g2f.EvaluatorMode.jacobian,
        outputs=jacobians,
        residuals=residuals,
        print=printout,
    )
    model_func_scope.verify_jacobian()
    loglike_jac = model_func_scope.evaluate()
...
def psf_models_linear_gaussians(channels, psf_models):
    ...
    for idx, psf_model in enumerate(psf_models):
        ...
        # Return the param to its original state
        params[0].fixed = True
    return gaussians
...
def test_modeller(model):
    ...
    model.setup_evaluators(evaluatormode=g2f.EvaluatorMode.loglike_image, force=True)
    ...
    for delta_param in (0, 0.2):
        ...
        # TODO: This should check >0, and < (some reasonable value)
        # However, but the scipy least squares does not do a great job
        # optimizing and the loglike_grad isn't even negative...
        ...
        for offset in (0, 1e-6):
            ...
            model_new = g2f.ModelD(
                data=model.data, psfmodels=model.psfmodels, sources=model.sources, priors=priors
            )
            model_new.setup_evaluators(evaluatormode=g2f.EvaluatorMode.loglike)
            loglike_init = sum(loglike_eval for loglike_eval in model_new.evaluate())
            if offset == 0:
            ...
            time_init = time.process_time()
            results = modeller.fit_model(model_new, **kwargs_fit)
            ...
            model_new.setup_evaluators(evaluatormode=g2f.EvaluatorMode.loglike)
            loglike_model = sum(loglike_eval for loglike_eval in model_new.evaluate())
    ...
    # Return parameters to original values since they are shared between
    # model instances, which is a useful persistence test.
    for param, value in zip(params_free, values_true):
        param.value = value
    ...
    for model in psf_fit_models:
        ...
        if errors:
            ...
            for diff, output in zip(diffs, outputs):
                ...
                plt.show()
        assert len(errors) == 0
        # Return the params to their original settings
        for param in params_freed:
            param.fixed = True
...
def test_modeller_fit_linear(model):
    ...
    params_other = {}
    for param in params:
        (params_linear_free if (param.free and param.linear) else params_other)[param] = param.value
    results = modeller.fit_model_linear(model, validate=True)
    assert results is not None

    for param, value in params_linear_free.items():
        assert value != param.value
        param.value = value
    assert all([param.value == value for param, value in params_other.items()])
